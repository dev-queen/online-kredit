...
class SxGeo
{
    ...
    public $additionalBasePath = false;

    public function __construct($db_file = 'SxGeo.dat', $type = SXGEO_FILE, $additionalBase = '')
    {
        if ($additionalBase)
            $this->additionalBasePath = $additionalBase;
    ...
    public function getCityFull($ip)
    {
        ...
        if ($seek) {
            $result = $this->parseCity($seek, 1);

            if ($this->additionalBasePath && $result['region']['id']) {
                $contentsOfFile = file_get_contents($this->additionalBasePath);
                //var_dump($result['region']['id']);
                $delimiter = "\n";
                $splitContents = explode($delimiter, $contentsOfFile);
                foreach ($splitContents as $line) {
                    $bits = explode("\t", $line);

                    if ($result['region']['id'] == $bits[0]) {
                        $result['timezone'] = $bits[5];
                        break;
                    }
                }
                return $result;
